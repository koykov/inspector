...
func (i11 TestStructInspector) Compare(src any, cond inspector.Op, right string, result *bool, path ...string) (err error) {
	...
	if len(path) > 0 {
		...
		if path[0] == "D" {
			...
			t400, err400 := strconv.ParseFloat(right, 0)
			if err400 != nil {
				return err400
			}
			rightExact = float64(t400)
			switch cond {
			case inspector.OpEq:
