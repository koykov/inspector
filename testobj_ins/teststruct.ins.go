...
func (i11 TestStructInspector) Cmp(src interface{}, cond inspector.Op, right string, result *bool, path ...string) (err error) {
	...
	if len(path) > 0 {
		...
		if path[0] == "F" {
			...
			t188, err188 := strconv.ParseFloat(right, 0)
			if err188 != nil {
				return err188
			}
			rightExact = float32(t188)
			switch cond {
			case inspector.OpEq:
		...
		if path[0] == "D" {
			...
			t189, err189 := strconv.ParseFloat(right, 0)
			if err189 != nil {
				return err189
			}
			rightExact = float64(t189)
			switch cond {
			case inspector.OpEq:
