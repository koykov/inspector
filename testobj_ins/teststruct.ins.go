...
func (i11 TestStructInspector) Cmp(src interface{}, cond inspector.Op, right string, result *bool, path ...string) (err error) {
	...
	if len(path) > 0 {
		...
		if path[0] == "D" {
			...
			t279, err279 := strconv.ParseFloat(right, 0)
			if err279 != nil {
				return err279
			}
			rightExact = float64(t279)
			switch cond {
			case inspector.OpEq:
