	...
	if len(path) > 0 {
		...
		if path[0] == "Permission" {
			...
			if len(path) > 1 {
				...
				var k int32
				t27, err27 := strconv.ParseInt(path[1], 0, 0)
				if err27 != nil {
					return err27
				}
				k = int32(t27)
		...
		if path[0] == "Finance" {
			...
			if len(path) > 1 {
				...
				if path[1] == "History" {
					...
					if len(path) > 2 {
						var i int
						t28, err28 := strconv.ParseInt(path[2], 0, 0)
						if err28 != nil {
							return err28
						}
						i = int(t28)
...
func (i5 TestObjectInspector) Compare(src any, cond inspector.Op, right string, result *bool, path ...string) (err error) {
	...
	if len(path) > 0 {
		...
		if path[0] == "Ustate" {
			...
			t32, err32 := strconv.ParseUint(right, 0, 0)
			if err32 != nil {
				return err32
			}
			rightExact = uint64(t32)
			switch cond {
			case inspector.OpEq:
		...
		if path[0] == "Finance" {
			...
			if len(path) > 1 {
				...
				if path[1] == "MoneyOut" {
					var rightExact float64
					t41, err41 := strconv.ParseFloat(right, 0)
					if err41 != nil {
						return err41
					}
					rightExact = float64(t41)
					switch cond {
					case inspector.OpEq:
				...
				if path[1] == "AllowBuy" {
					var rightExact bool
					t43, err43 := strconv.ParseBool(right)
					if err43 != nil {
						return err43
					}
					rightExact = bool(t43)
				...
				if path[1] == "History" {
					...
					if len(path) > 2 {
						var i int
						t44, err44 := strconv.ParseInt(path[2], 0, 0)
						if err44 != nil {
							return err44
						}
						i = int(t44)
						if len(x1) > i {
							...
							if len(path) > 3 {
								...
								if path[3] == "Cost" {
									var rightExact float64
									t46, err46 := strconv.ParseFloat(right, 0)
									if err46 != nil {
										return err46
									}
									rightExact = float64(t46)
									switch cond {
									case inspector.OpEq:
	...
	if path[0] == "Finance" {
		if x.Finance != nil {
			...
			if path[1] == "History" {
				...
				var i int
				t51, err51 := strconv.ParseInt(path[2], 0, 0)
				if err51 != nil {
					return err51
				}
				i = int(t51)
	...
	if path[0] == "Finance" {
		if x.Finance != nil {
			...
			if path[1] == "History" {
				...
				var i int
				t53, err53 := strconv.ParseInt(path[2], 0, 0)
				if err53 != nil {
					return err53
				}
				i = int(t53)
