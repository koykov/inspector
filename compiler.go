...
func NewCompiler(pkg, out string, w ByteStringWriter, l Logger) *Compiler {
	c := Compiler{
		pkg:    pkg,
		pkgDot: pkg + ".",
		outDir: out,
...
func (c *Compiler) Compile() error {
	...
	// todo make check if out dir is empty and writable

	err = c.writeInit()
	if err != nil {
		return err
	}
	err = c.writeFile(c.outDir + string(os.PathSeparator) + "init.go")
	if err != nil {
		return err
	}
	...
	for _, node := range c.nodes {
		...
		err = c.writeFile(c.outDir + string(os.PathSeparator) + strings.ToLower(node.name) + ".ins.go")
		if err != nil {
			return err
		}
	}
